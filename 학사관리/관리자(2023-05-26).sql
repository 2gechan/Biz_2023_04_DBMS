 ...
 -- SYSDBA 사용자는 데이터베이스 시스템을 생성, 제거, 파괴할 수 있는 권한을 가진 사용자
 ...
 -- student TABLESPACE 를 생성하고 저장소는 stduent.dbf 로 하라
 -- 초기 용량은 1MByte 로 설정하고 1KByte 씩 자동 증가하도록 설정하라
 CREATE TABLESPACE student -- 저장소의 이름을 student 라는 이름으로 사용하겠다
 DATAFILE 'C:/app/data/student.dbf' -- 저장소 폴더에 student.dbf 라는 이름으로 생성
 SIZE 1M AUTOEXTEND ON NEXT 1K; -- 초기의 저장소 공간은 1M(byte)확보하고
 ...
 -- 이러한 정책은 보안적인 면에서 권장하지만 때로는 불편할 때가 있다
 -- 일부 프로그래밍 언어에서 DB 에 접속할 때 ## 와 같은 특수문자가 있으면
 -- 접속에 문제를 일으키는 경우가 있다.
 -- 오라클에서는 12c 이후에 사용자 생성 정책을 
 -- 예전 방식으로 사용할 수 있도록 하는 설정을 제공한다
 ...
 --     심지어 DBMS 소프트웨어에 로그인 할 수도 없다
 -- 사용자에게 권한을 부여하여 기능을 활성화 시켜줘야 한다.
 -- 원칙적으로 권한 부여는 각각 항목별로 세부적으로 부여해야 하지만
 -- 학습적 편리성을 위하여 모든 권한을 한 번에 부여 할 것 이다
